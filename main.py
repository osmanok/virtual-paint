...
def get_contours(img_output):
    contours, hierarchy = cv2.findContours(img_output, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    x, y, w, h = 0, 0, 0, 0
    for cnt in contours:
...
def draw_on_canvas(my_points, my_color_values):
    for point in my_points:
        cv2.circle(imgResult, (point[0], point[1]), 10, my_color_values[point[2]], cv2.FILLED)
